  ...
  if (assessment.score.kolb) {
    ...
    (assessment.type as any) === 'adult-thinking'
      ...
      if (assessment.score[typeKey] && assessment.score[typeKey].scores) {
          ...
          style = assessment.score[typeKey].personalityType || assessment.score[typeKey].dominantStyle || 'Balanced';
          ...
          // Determine dominant style manually
          const entries = Object.entries(actualScores).filter(([k]) => typeof actualScores[k as keyof typeof actualScores] === 'number');
          if (entries.length > 0) {
      ...
      return generateModernThinkingInsights(style, actualScores);
  }
...
export function generateModernThinkingInsights(style: string, scores: any): AssessmentInsights {
  const insights: AssessmentInsights = {
    strengths: [],
    weaknesses: [],
    improvements: [],
  ...
  switch (style) {
    ...
    case 'Reflective':
      ...
      insights.weaknesses = [
        'May struggle in high-pressure, time-sensitive situations',
        'Can overthink and suffer from analysis paralysis',
        'Might miss fleeting opportunities',
        'May appear indecisive to action-oriented colleagues',
        'Can demand too much information before deciding'
      ];
      insights.improvements = [
        'Set strict time limits for gathering information',
        'Practice making lower-stakes decisions quickly',
        'Develop trust in your initial intuitions',
        'Learn to identify when a "good enough" decision is needed',
        'Communicate your need for processing time clearly'
      ];
      insights.organizationalFit = [
        'Ideal for: Strategic planning, research, risk management, policy development',
        'Team contribution: Prevents rushed mistakes and ensures thoroughness',
        'Leadership style: Thoughtful, deliberate, consensus-oriented'
      ];
      insights.continuousReview = [
        'Monitor: Tendency to delay necessary actions',
        'Develop: Comfort with making decisions on partial information',
        'Leverage: Deep analytical thinking for long-term strategy'
      ];
      break;

    default: // Balanced
      insights.strengths = [
        'Highly adaptable across different thinking styles',
        'Able to balance creativity, analysis, and practicality',
        'Excellent team player who bridges gaps between different types of thinkers',
        'Well-rounded approach to problem solving'
      ];
      insights.weaknesses = [
        'May not naturally gravitate towards specialized roles',
        'Can occasionally struggle when a singular extreme focus is required'
      ];
      insights.improvements = [
        'Lean into specific thinking styles when the situation demands it',
        'Act as a mediator in team settings',
        'Leverage your versatility'
      ];
      insights.organizationalFit = [
        'Ideal for: Leadership, general management, cross-functional roles',
        'Team contribution: Acts as a bridge between specialized team members',
        'Leadership style: Balanced, adaptable, comprehensive'
      ];
      insights.continuousReview = [
        'Monitor: Ability to shift gears when needed',
        'Develop: Deeper expertise in one specific style if desired',
        'Leverage: Your natural versatility in complex projects'
      ];
      break;
  }
